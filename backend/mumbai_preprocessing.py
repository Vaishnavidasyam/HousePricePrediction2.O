...
class MumbaiHousePreprocessor:
    ...
    def _parse_area(self, s: str):
        if not isinstance(s, str):
            return None
        # Similar style to Hyderabad AREA field
        s = (
            s.replace("sq.ft.", "")
            .replace("sq.ft", "")
            .replace("sqft", "")
    ...
    def _parse_price(self, s: str):
        # Same logic as Hyderabad: "1.2 Cr", "50 L", ranges, etc.
        if not isinstance(s, str):
            return None
        s = s.strip()
    ...
    def load_and_clean(self):
        ...
        print("Mumbai columns:", df.columns.tolist())

        required_cols = ["location", "BEDROOM_NUM", "AREA", "PRICE"]
        missing = [c for c in required_cols if c not in df.columns]
        if missing:
            raise ValueError(f"Mumbai missing required columns: {missing}")

        df = df[required_cols].copy()
        print("Mumbai: selected ['location','BEDROOM_NUM','AREA','PRICE']:", df.shape)
        ...
        print("Mumbai: after parsing & dropping NaN:", df.shape)

        # sensible ranges
        df = df[(df["AREA_NUM"] >= 20) & (df["AREA_NUM"] <= 5000)]
        df = df[df["PRICE_NUM"] > 0]
        print("Mumbai: after filtering area & price:", df.shape)
        ...
        print("Mumbai: after keeping only 2BHK & 3BHK:", df.shape)
    ...
    def encode_locality(self, locality):
        ...
        if locality not in self.locality_encoder.classes_:
            raise ValueError(f"Mumbai locality '{locality}' not seen during training.")
